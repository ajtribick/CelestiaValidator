...
_TEXTURE_EXTENSIONS = [".jpg", ".jpeg", ".png", ".dds", ".dxt5nm", ".ctx", ".avif", ".*"]
...
def is_file(filename: str) -> bool:
    ...
    return (
        ...
        filename not in ('CON', 'PRN', 'AUX', 'NUL') and
        _INVALID_FILE_REGEX.match(filename) is None
    )

def is_mesh_file(filename: str) -> bool:
    """Checks if a filename is a mesh file"""
    return is_file(filename) and os.path.splitext(filename)[1].casefold() in _MESH_EXTENSIONS

def is_texture_file(filename: str) -> bool:
    """Checks if a filename is a texture file"""
    return is_file(filename) and os.path.splitext(filename)[1].casefold() in _TEXTURE_EXTENSIONS

def is_trajectory_file(filename: str) -> bool:
    """Checks if a filename is a trajectory file"""
    return is_file(filename) and os.path.splitext(filename)[1].casefold() in _TRAJECTORY_EXTENSIONS
