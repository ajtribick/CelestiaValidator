...
from celvalidate import DSCParser, MessageLevel, ParsingError, SSCParser, STCParser, TokenFileParser
...
def _process_messages(filename: pathlib.Path, parser: TokenFileParser, is_verbose: bool) -> int:
    exit_code = 0
...
def _process_file(text: TextIO, filename: str, parser_type: type, is_verbose: bool) -> int:
    "Process a DSC file"
...
def _process_directory(path: pathlib.Path, is_verbose: bool) -> int:
    ...
    for file in path.rglob('*.*'):
        if (parser_type := _PARSER_MAPPING.get(file.suffix.casefold(), None)) is not None:
            relative_name = file.relative_to(path)
            with open(file, 'rt', encoding='utf-8', errors='replace') as f:
                exit_code = max(exit_code, _process_file(f, relative_name, parser_type, is_verbose))
    return exit_code
...
def _process_archive(path: pathlib.Path, is_verbose: bool) -> int:
    ...
    with zipfile.ZipFile(path, mode='r') as zf:
        for zipinfo in zf.infolist():
            ...
            if (parser_type := _PARSER_MAPPING.get(file_path.suffix.casefold(), None)) is None:
                continue
            with file_path.open(mode='rt', encoding='utf-8', errors='replace') as f:
                exit_code = max(
                    exit_code, _process_file(f, zipinfo.filename, parser_type, is_verbose)
                )
    return exit_code
...
argparser.add_argument('path', type=pathlib.Path, help='path to file, directory or archive')
argparser.add_argument(
    '-v', '--verbose', action='store_true', help='display additional informational messages'
)
